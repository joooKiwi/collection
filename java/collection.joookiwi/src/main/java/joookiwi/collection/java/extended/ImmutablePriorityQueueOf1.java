...
@NotNullByDefault
public class ImmutablePriorityQueueOf1<T>
        extends ImmutablePriorityQueue<T>
        implements ClassWith1Value<T> {

    //#region -------------------- Fields --------------------

    @Serial private static final long serialVersionUID = 4699026564984566468L;
    ...
    public ImmutablePriorityQueueOf1(final T value) {
        super();
        __value = value;
    }

    public ImmutablePriorityQueueOf1(final T value, final @Nullable Comparator<? super T> comparator) {
        super(comparator);
        __value = value;
    }
    ...
    @Contract(ALWAYS_NEW_0)
    @Override public ImmutablePriorityQueueOf1<T> clone() {
        return (ImmutablePriorityQueueOf1<T>) super.clone();
    }
