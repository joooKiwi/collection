...
public class MutableArrayBlockingQueue<T>
    ...
    /// and [no fairness][joookiwi.collection.java.CollectionConstants#DEFAULT_FAIRNESS]
    public MutableArrayBlockingQueue(final byte capacity) { super(min(capacity, 1), DEFAULT_FAIRNESS); }

    /// Create an empty mutable instance of [ArrayBlockingQueue]
    /// with a capacity received (or [Integer#MAX_VALUE] if it is `null`)
    /// and [no fairness][joookiwi.collection.java.CollectionConstants#DEFAULT_FAIRNESS]
    public MutableArrayBlockingQueue(final @Nullable Byte capacity) { super(capacity == null ? MAX_INT_VALUE : min(capacity.byteValue(), 1), DEFAULT_FAIRNESS); }

    /// Create an empty mutable instance of [ArrayBlockingQueue]
    /// with a capacity received
    /// and [no fairness][joookiwi.collection.java.CollectionConstants#DEFAULT_FAIRNESS]
    public MutableArrayBlockingQueue(final short capacity) { super(min(capacity, (short) 1), DEFAULT_FAIRNESS); }

    /// Create an empty mutable instance of [ArrayBlockingQueue]
    /// with a capacity received (or [Integer#MAX_VALUE] if it is `null`)
    /// and [no fairness][joookiwi.collection.java.CollectionConstants#DEFAULT_FAIRNESS]
    public MutableArrayBlockingQueue(final @Nullable Short capacity) { super(capacity == null ? MAX_INT_VALUE : min(capacity.shortValue(), 1), DEFAULT_FAIRNESS); }

    /// Create an empty mutable instance of [ArrayBlockingQueue]
    /// with a capacity received
    /// and [no fairness][joookiwi.collection.java.CollectionConstants#DEFAULT_FAIRNESS]
    public MutableArrayBlockingQueue(final int capacity) { super(min(capacity, 1), DEFAULT_FAIRNESS); }

    /// Create an empty mutable instance of [ArrayBlockingQueue]
    /// with a capacity received (or [Integer#MAX_VALUE] if it is `null`)
    /// and [no fairness][joookiwi.collection.java.CollectionConstants#DEFAULT_FAIRNESS]
    public MutableArrayBlockingQueue(final @Nullable Integer capacity) { super(capacity == null ? MAX_INT_VALUE : min(capacity.intValue(), 1), DEFAULT_FAIRNESS); }
    ...
    public MutableArrayBlockingQueue(final byte capacity,
                                     final boolean isFair) { super(min(capacity, 1), isFair); }

    /// Create an empty mutable instance of [ArrayBlockingQueue]
    /// with a capacity received
    /// and the fairness received (or [no fairness][joookiwi.collection.java.CollectionConstants#DEFAULT_FAIRNESS] if it is `null`)
    public MutableArrayBlockingQueue(final byte capacity,
                                     final @Nullable Boolean isFair) { super(min(capacity, 1), isFair == null ? DEFAULT_FAIRNESS : isFair); }


    /// Create an empty mutable instance of [ArrayBlockingQueue]
    /// with a capacity received (or [Integer#MAX_VALUE] if it is `null`)
    /// and the fairness received
    public MutableArrayBlockingQueue(final @Nullable Byte capacity,
                                     final boolean isFair) { super(capacity == null ? MAX_INT_VALUE : min(capacity.byteValue(), 1), isFair); }

    /// Create an empty mutable instance of [ArrayBlockingQueue]
    /// with a capacity received (or [Integer#MAX_VALUE] if it is `null`)
    /// and the fairness received (or [no fairness][joookiwi.collection.java.CollectionConstants#DEFAULT_FAIRNESS] if it is `null`)
    public MutableArrayBlockingQueue(final @Nullable Byte capacity,
                                     final @Nullable Boolean isFair) { super(capacity == null ? MAX_INT_VALUE : min(capacity.byteValue(), 1), isFair == null ? DEFAULT_FAIRNESS : isFair); }


    /// Create an empty mutable instance of [ArrayBlockingQueue]
    /// with a capacity received
    /// and the fairness received
    public MutableArrayBlockingQueue(final short capacity,
                                     final boolean isFair) { super(min(capacity, 1), isFair); }
    ...
    /// with a capacity received (or [Integer#MAX_VALUE] if it is `null`)
    /// and the fairness received
    public MutableArrayBlockingQueue(final @Nullable Short capacity,
                                     final boolean isFair) { super(capacity == null ? MAX_INT_VALUE : min(capacity.shortValue(), 1), isFair); }

    /// Create an empty mutable instance of [ArrayBlockingQueue]
    /// with a capacity received (or [Integer#MAX_VALUE] if it is `null`)
    /// and the fairness received (or [no fairness][joookiwi.collection.java.CollectionConstants#DEFAULT_FAIRNESS] if it is `null`)
    public MutableArrayBlockingQueue(final @Nullable Short capacity,
                                     final @Nullable Boolean isFair) { super(capacity == null ? MAX_INT_VALUE : min(capacity.shortValue(), 1), isFair == null ? DEFAULT_FAIRNESS : isFair); }


    /// Create an empty mutable instance of [ArrayBlockingQueue]
    /// with a capacity received
    /// and the fairness received
    public MutableArrayBlockingQueue(final int capacity,
                                     final boolean isFair) { super(min(capacity, 1), isFair); }
    ...
    /// with a capacity received (or [Integer#MAX_VALUE] if it is `null`)
    /// and the fairness received
    public MutableArrayBlockingQueue(final @Nullable Integer capacity,
                                     final boolean isFair) { super(capacity == null ? MAX_INT_VALUE : min(capacity.intValue(), 1), isFair); }

    /// Create an empty mutable instance of [ArrayBlockingQueue]
    /// with a capacity received (or [Integer#MAX_VALUE] if it is `null`)
    /// and the fairness received (or [no fairness][joookiwi.collection.java.CollectionConstants#DEFAULT_FAIRNESS] if it is `null`)
    public MutableArrayBlockingQueue(final @Nullable Integer capacity,
                                     final @Nullable Boolean isFair) { super(capacity == null ? MAX_INT_VALUE : min(capacity.intValue(), 1), isFair == null ? DEFAULT_FAIRNESS : isFair); }
    ...
    /// and [no fairness][joookiwi.collection.java.CollectionConstants#DEFAULT_FAIRNESS]
    public MutableArrayBlockingQueue(final @Flow(sourceIsContainer = true, targetIsContainer = true) T @Unmodifiable [] values,
                                     final byte capacity) { super(max(values.length, min(capacity, 1)), DEFAULT_FAIRNESS, toCollection(values)); }

    /// Create a mutable instance of [ArrayBlockingQueue]
    /// with a capacity received (or [Integer#MAX_VALUE] if it is `null`)
    /// and [no fairness][joookiwi.collection.java.CollectionConstants#DEFAULT_FAIRNESS]
    public MutableArrayBlockingQueue(final @Flow(sourceIsContainer = true, targetIsContainer = true) T @Unmodifiable [] values,
                                     final @Nullable Byte capacity) { super(capacity == null ? MAX_INT_VALUE : max(values.length, min(capacity.byteValue(), 1)), DEFAULT_FAIRNESS, toCollection(values)); }

    /// Create a mutable instance of [ArrayBlockingQueue]
    /// with a capacity received
    /// and [no fairness][joookiwi.collection.java.CollectionConstants#DEFAULT_FAIRNESS]
    public MutableArrayBlockingQueue(final @Flow(sourceIsContainer = true, targetIsContainer = true) T @Unmodifiable [] values,
                                     final short capacity) { super(max(values.length, min(capacity, 1)), DEFAULT_FAIRNESS, toCollection(values)); }

    /// Create a mutable instance of [ArrayBlockingQueue]
    /// with a capacity received (or [Integer#MAX_VALUE] if it is `null`)
    /// and [no fairness][joookiwi.collection.java.CollectionConstants#DEFAULT_FAIRNESS]
    public MutableArrayBlockingQueue(final @Flow(sourceIsContainer = true, targetIsContainer = true) T @Unmodifiable [] values,
                                     final @Nullable Short capacity) { super(capacity == null ? MAX_INT_VALUE : max(values.length, min(capacity.shortValue(), 1)), DEFAULT_FAIRNESS, toCollection(values)); }

    /// Create a mutable instance of [ArrayBlockingQueue]
    /// with a capacity received
    /// and [no fairness][joookiwi.collection.java.CollectionConstants#DEFAULT_FAIRNESS]
    public MutableArrayBlockingQueue(final @Flow(sourceIsContainer = true, targetIsContainer = true) T @Unmodifiable [] values,
                                     final int capacity) { super(max(values.length, min(capacity, 1)), DEFAULT_FAIRNESS, toCollection(values)); }

    /// Create a mutable instance of [ArrayBlockingQueue]
    /// with a capacity received (or [Integer#MAX_VALUE] if it is `null`)
    /// and [no fairness][joookiwi.collection.java.CollectionConstants#DEFAULT_FAIRNESS]
    public MutableArrayBlockingQueue(final @Flow(sourceIsContainer = true, targetIsContainer = true) T @Unmodifiable [] values,
                                     final @Nullable Integer capacity) { super(capacity == null ? MAX_INT_VALUE : max(values.length, min(capacity.intValue(), 1)), DEFAULT_FAIRNESS, toCollection(values)); }
    ...
    /// and [no fairness][joookiwi.collection.java.CollectionConstants#DEFAULT_FAIRNESS]
    public MutableArrayBlockingQueue(final @Flow(sourceIsContainer = true, targetIsContainer = true) @Unmodifiable Collection<? extends T> values,
                                     final byte capacity) { super(max(values.size(), min(capacity, 1)), DEFAULT_FAIRNESS, values); }

    /// Create a mutable instance of [ArrayBlockingQueue]
    /// with a capacity received (or [Integer#MAX_VALUE] if it is `null`)
    /// and [no fairness][joookiwi.collection.java.CollectionConstants#DEFAULT_FAIRNESS]
    public MutableArrayBlockingQueue(final @Flow(sourceIsContainer = true, targetIsContainer = true) @Unmodifiable Collection<? extends T> values,
                                     final @Nullable Byte capacity) { super(capacity == null ? MAX_INT_VALUE : max(values.size(), min(capacity.byteValue(), 1)), DEFAULT_FAIRNESS, values); }

    /// Create a mutable instance of [ArrayBlockingQueue]
    /// with a capacity received
    /// and [no fairness][joookiwi.collection.java.CollectionConstants#DEFAULT_FAIRNESS]
    public MutableArrayBlockingQueue(final @Flow(sourceIsContainer = true, targetIsContainer = true) @Unmodifiable Collection<? extends T> values,
                                     final short capacity) { super(max(values.size(), min(capacity, 1)), DEFAULT_FAIRNESS, values); }

    /// Create a mutable instance of [ArrayBlockingQueue]
    /// with a capacity received (or [Integer#MAX_VALUE] if it is `null`)
    /// and [no fairness][joookiwi.collection.java.CollectionConstants#DEFAULT_FAIRNESS]
    public MutableArrayBlockingQueue(final @Flow(sourceIsContainer = true, targetIsContainer = true) @Unmodifiable Collection<? extends T> values,
                                     final @Nullable Short capacity) { super(capacity == null ? MAX_INT_VALUE : max(values.size(), min(capacity.shortValue(), 1)), DEFAULT_FAIRNESS, values); }

    /// Create a mutable instance of [ArrayBlockingQueue]
    /// with a capacity received
    /// and [no fairness][joookiwi.collection.java.CollectionConstants#DEFAULT_FAIRNESS]
    public MutableArrayBlockingQueue(final @Flow(sourceIsContainer = true, targetIsContainer = true) @Unmodifiable Collection<? extends T> values,
                                     final int capacity) { super(max(values.size(), min(capacity, 1)), DEFAULT_FAIRNESS, values); }

    /// Create a mutable instance of [ArrayBlockingQueue]
    /// with a capacity received (or [Integer#MAX_VALUE] if it is `null`)
    /// and [no fairness][joookiwi.collection.java.CollectionConstants#DEFAULT_FAIRNESS]
    public MutableArrayBlockingQueue(final @Flow(sourceIsContainer = true, targetIsContainer = true) @Unmodifiable Collection<? extends T> values,
                                     final @Nullable Integer capacity) { super(capacity == null ? MAX_INT_VALUE : max(values.size(), min(capacity.intValue(), 1)), DEFAULT_FAIRNESS, values); }
    ...
    public MutableArrayBlockingQueue(final @Flow(sourceIsContainer = true, targetIsContainer = true) T @Unmodifiable [] values,
                                     final byte capacity,
                                     final boolean isFair) { super(max(values.length, min(capacity, 1)), isFair, toCollection(values)); }
    ...
    public MutableArrayBlockingQueue(final @Flow(sourceIsContainer = true, targetIsContainer = true) T @Unmodifiable [] values,
                                     final byte capacity,
                                     final @Nullable Boolean isFair) { super(max(values.length, min(capacity, 1)), isFair == null ? DEFAULT_FAIRNESS : isFair, toCollection(values)); }
    ...
    public MutableArrayBlockingQueue(final
                                     ...
                                     final boolean isFair) { super(capacity == null ? MAX_INT_VALUE : max(values.length, min(capacity.byteValue(), 1)), isFair, toCollection(values)); }
    ...
    public MutableArrayBlockingQueue(final @Flow(sourceIsContainer = true, targetIsContainer = true) T @Unmodifiable [] values,
                                     final @Nullable Byte capacity,
                                     final @Nullable Boolean isFair) { super(capacity == null ? MAX_INT_VALUE : max(values.length, min(capacity.byteValue(), 1)), isFair == null ? DEFAULT_FAIRNESS : isFair, toCollection(values)); }
    ...
    public MutableArrayBlockingQueue(final @Flow(sourceIsContainer = true, targetIsContainer = true) T @Unmodifiable [] values,
                                     ...
                                     final boolean isFair) { super(capacity == null ? MAX_INT_VALUE : max(values.length, min(capacity.shortValue(), 1)), isFair, toCollection(values)); }
    ...
    public MutableArrayBlockingQueue(final @Flow(sourceIsContainer = true, targetIsContainer = true) T @Unmodifiable [] values,
                                     final @Nullable Short capacity,
                                     final @Nullable Boolean isFair) { super(capacity == null ? MAX_INT_VALUE : max(values.length, min(capacity.shortValue(), 1)), isFair == null ? DEFAULT_FAIRNESS : isFair, toCollection(values)); }
    ...
    public MutableArrayBlockingQueue(final @Flow(sourceIsContainer = true, targetIsContainer = true) T @Unmodifiable [] values,
                                     ...
                                     final boolean isFair) { super(capacity == null ? MAX_INT_VALUE : max(values.length, min(capacity.intValue(), 1)), isFair, toCollection(values)); }
    ...
    public MutableArrayBlockingQueue(final @Flow(sourceIsContainer = true, targetIsContainer = true) T @Unmodifiable [] values,
                                     final @Nullable Integer capacity,
                                     final @Nullable Boolean isFair) { super(capacity == null ? MAX_INT_VALUE : max(values.length, min(capacity.intValue(), 1)), isFair == null ? DEFAULT_FAIRNESS : isFair, toCollection(values)); }
    ...
    public MutableArrayBlockingQueue(final @Flow(sourceIsContainer = true, targetIsContainer = true) @Unmodifiable Collection<? extends T> values,
                                     final byte capacity,
                                     final boolean isFair) { super(max(values.size(), min(capacity, 1)), isFair, values); }
    ...
    public MutableArrayBlockingQueue(final @Flow(sourceIsContainer = true, targetIsContainer = true) @Unmodifiable Collection<? extends T> values,
                                     final byte capacity,
                                     final @Nullable Boolean isFair) { super(max(values.size(), min(capacity, 1)), isFair == null ? DEFAULT_FAIRNESS : isFair, values); }
    ...
    public MutableArrayBlockingQueue(final @Flow(sourceIsContainer = true, targetIsContainer = true) @Unmodifiable Collection<? extends T> values,
                                     final @Nullable Byte capacity,
                                     final boolean isFair) { super(capacity == null ? MAX_INT_VALUE : max(values.size(), min(capacity.byteValue(), 1)), isFair, values); }
    ...
    public MutableArrayBlockingQueue(final @Flow(sourceIsContainer = true, targetIsContainer = true) @Unmodifiable Collection<? extends T> values,
                                     final @Nullable Byte capacity,
                                     final @Nullable Boolean isFair) { super(capacity == null ? MAX_INT_VALUE : max(values.size(), min(capacity.byteValue(), 1)), isFair == null ? DEFAULT_FAIRNESS : isFair, values); }
    ...
    public MutableArrayBlockingQueue(final @Flow(sourceIsContainer = true, targetIsContainer = true) @Unmodifiable Collection<? extends T> values,
                                     final @Nullable Short capacity,
                                     final boolean isFair) { super(capacity == null ? MAX_INT_VALUE : max(values.size(), min(capacity.shortValue(), 1)), isFair, values); }
    ...
    public MutableArrayBlockingQueue(final @Flow(sourceIsContainer = true, targetIsContainer = true) @Unmodifiable Collection<? extends T> values,
                                     final @Nullable Short capacity,
                                     final @Nullable Boolean isFair) { super(capacity == null ? MAX_INT_VALUE : max(values.size(), min(capacity.shortValue(), 1)), isFair == null ? DEFAULT_FAIRNESS : isFair, values); }
    ...
    public MutableArrayBlockingQueue(final @Flow(sourceIsContainer = true, targetIsContainer = true) @Unmodifiable Collection<? extends T> values,
                                     final @Nullable Integer capacity,
                                     final boolean isFair) { super(capacity == null ? MAX_INT_VALUE : max(values.size(), min(capacity.intValue(), 1)), isFair, values); }
    ...
    public MutableArrayBlockingQueue(final @Flow(sourceIsContainer = true, targetIsContainer = true) @Unmodifiable Collection<? extends T> values,
                                     final @Nullable Integer capacity,
                                     final @Nullable Boolean isFair) { super(capacity == null ? MAX_INT_VALUE : max(values.size(), min(capacity.intValue(), 1)), isFair == null ? DEFAULT_FAIRNESS : isFair, values); }
