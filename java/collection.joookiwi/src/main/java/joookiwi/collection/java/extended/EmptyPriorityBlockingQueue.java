...
/// A [Singleton] implementation of the [java.util.concurrent.PriorityBlockingQueue],
/// but in an empty form
///
/// @param <T> The type of the element
/// @see joookiwi.collection.java.CollectionConstants#emptyPriorityBlockingQueue
@Singleton
@NotNullByDefault
public class EmptyPriorityBlockingQueue<T>
        extends ImmutablePriorityBlockingQueue<T> {

    @Serial private static final long serialVersionUID = 8550873300907270774L;

    //#region -------------------- Singleton usage --------------------

    private static volatile @Nullable EmptyPriorityBlockingQueue<?> __instance;

    protected EmptyPriorityBlockingQueue() { super(); }

    @SuppressWarnings("unchecked cast")
    public static <T> EmptyPriorityBlockingQueue<T> getInstance() {
        final var instance = __instance;
        if (instance != null)
            return (EmptyPriorityBlockingQueue<T>) instance;
        synchronized (EmptyPriorityBlockingQueue.class) {
            final var instance2 = __instance;
            if (instance2 != null)
                return (EmptyPriorityBlockingQueue<T>) instance2;
            return (EmptyPriorityBlockingQueue<T>) (__instance = new EmptyPriorityBlockingQueue<>());
        }
    }
    ...
    @Contract(value = ALWAYS_THIS_0, pure = true) @Override public EmptyPriorityBlockingQueue<T> clone() { return this; }
