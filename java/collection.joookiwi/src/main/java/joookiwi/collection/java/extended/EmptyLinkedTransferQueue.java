...
/// A [Singleton] implementation of the [java.util.concurrent.LinkedTransferQueue],
...
public class EmptyLinkedTransferQueue<T>
    ...
    @Serial private static final long serialVersionUID = -4534948684837738867L;

    //#region -------------------- Singleton usage --------------------

    private static volatile @Nullable EmptyLinkedTransferQueue<?> __instance;

    protected EmptyLinkedTransferQueue() { super(); }

    @SuppressWarnings("unchecked cast")
    public static <T> EmptyLinkedTransferQueue<T> getInstance() {
        final var instance = __instance;
        if (instance != null)
            return (EmptyLinkedTransferQueue<T>) instance;
        synchronized (EmptyLinkedTransferQueue.class) {
            final var instance2 = __instance;
            if (instance2 != null)
                return (EmptyLinkedTransferQueue<T>) instance2;
            return (EmptyLinkedTransferQueue<T>) (__instance = new EmptyLinkedTransferQueue<>());
        }
    }
    ...
    @Contract(value = ALWAYS_FAIL_0, pure = true) @Override public T element() { throw new NoSuchElementException("No element could be found in an empty LinkedTransferQueue."); }
    ...
    @Contract(value = ALWAYS_THIS_0, pure = true) @Override public EmptyLinkedTransferQueue<T> clone() { return this; }
