...
/// A [Singleton] implementation of the [java.util.concurrent.ArrayBlockingQueue],
...
public class EmptyArrayBlockingQueue<T>
    ...
    @Serial private static final long serialVersionUID = -5894772340151650121L;

    //#region -------------------- Singleton usage --------------------

    private static volatile @Nullable EmptyArrayBlockingQueue<?> __instance;

    protected EmptyArrayBlockingQueue() { super(); }

    @SuppressWarnings("unchecked cast")
    public static <T> EmptyArrayBlockingQueue<T> getInstance() {
        final var instance = __instance;
        if (instance != null)
            return (EmptyArrayBlockingQueue<T>) instance;
        synchronized (EmptyArrayBlockingQueue.class) {
            final var instance2 = __instance;
            if (instance2 != null)
                return (EmptyArrayBlockingQueue<T>) instance2;
            return (EmptyArrayBlockingQueue<T>) (__instance = new EmptyArrayBlockingQueue<>());
        }
    }
    ...
    @Contract(value = ALWAYS_FAIL_0, pure = true) @Override public T element() { throw new NoSuchElementException("No element could be found in an empty ArrayBlockingQueue."); }
