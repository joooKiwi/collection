...
/// A [Singleton] implementation of the [java.util.concurrent.DelayQueue],
...
public class EmptyDelayQueue<T extends Delayed>
    ...
    @Serial private static final long serialVersionUID = -6010559266602594452L;

    //#region -------------------- Singleton usage --------------------

    private static volatile @Nullable EmptyDelayQueue<?> __instance;

    protected EmptyDelayQueue() { super(); }

    @SuppressWarnings("unchecked cast")
    public static <T extends Delayed> EmptyDelayQueue<T> getInstance() {
        final var instance = __instance;
        if (instance != null)
            return (EmptyDelayQueue<T>) instance;
        synchronized (EmptyDelayQueue.class) {
            final var instance2 = __instance;
            if (instance2 != null)
                return (EmptyDelayQueue<T>) instance2;
            return (EmptyDelayQueue<T>) (__instance = new EmptyDelayQueue<>());
        }
    }
    ...
    @Contract(value = ALWAYS_FAIL_0, pure = true) @Override public T element() { throw new NoSuchElementException("No element could be found in an empty DelayQueue."); }
