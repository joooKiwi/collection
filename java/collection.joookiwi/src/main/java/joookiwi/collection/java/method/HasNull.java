...
public final class HasNull
    ...
    public static <T> boolean hasNull(@Nullable MinimalistCollectionHolder<? extends T> collection) {
        if (collection == null)
            return false;
    ...
    @ExtensionFunction
    @Contract(IF_1ST_NULL_THEN_FALSE_1)
    public static <T> boolean hasNull(@Nullable CollectionHolder<? extends T> collection) {
        if (collection == null)
            return false;
    ...
    private static boolean __hasNull(@NotNull MinimalistCollectionHolder<?> collection, int size) {
        var index = -1;
        while (++index < size)
