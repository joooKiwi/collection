...
/// A [Singleton] implementation of the [LinkedBlockingDeque],
/// but in an empty form
///
/// @param <T> The type of the element
/// @see joookiwi.collection.java.CollectionConstants#emptyLinkedBlockingDeque
@Singleton
@NotNullByDefault
public class EmptyLinkedBlockingDeque<T>
        extends ImmutableLinkedBlockingDeque<T> {

    @Serial private static final long serialVersionUID = -2647408745567021837L;

    //#region -------------------- Singleton usage --------------------

    private static volatile @Nullable EmptyLinkedBlockingDeque<?> __instance;

    protected EmptyLinkedBlockingDeque() { super(); }

    @SuppressWarnings("unchecked cast")
    public static <T> EmptyLinkedBlockingDeque<T> getInstance() {
        final var instance = __instance;
        if (instance != null)
            return (EmptyLinkedBlockingDeque<T>) instance;
        synchronized (EmptyLinkedBlockingDeque.class) {
            final var instance2 = __instance;
            if (instance2 != null)
                return (EmptyLinkedBlockingDeque<T>) instance2;
            return (EmptyLinkedBlockingDeque<T>) (__instance = new EmptyLinkedBlockingDeque<>());
        }
    }
    ...
    @Override public T element() { throw new NoSuchElementException("No element could be found in an empty LinkedBlockingDeque."); }
    ...
    @Contract(value = ALWAYS_THIS_0, pure = true) @Override public EmptyLinkedBlockingDeque<T> reversed() { return this; }
