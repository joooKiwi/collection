...
/// A [Singleton] implementation of the [java.util.concurrent.ConcurrentLinkedQueue],
/// but in an empty form
///
/// @param <T> The type of the element
/// @see joookiwi.collection.java.CollectionConstants#emptyConcurrentLinkedQueue
@Singleton
@NotNullByDefault
public class EmptyConcurrentLinkedQueue<T>
        extends ImmutableConcurrentLinkedQueue<T> {

    @Serial private static final long serialVersionUID = 6240300484536915368L;

    //#region -------------------- Singleton usage --------------------

    private static volatile @Nullable EmptyConcurrentLinkedQueue<?> __instance;

    protected EmptyConcurrentLinkedQueue() { super(); }

    @SuppressWarnings("unchecked cast")
    public static <T> EmptyConcurrentLinkedQueue<T> getInstance() {
        final var instance = __instance;
        if (instance != null)
            return (EmptyConcurrentLinkedQueue<T>) instance;
        synchronized (EmptyConcurrentLinkedQueue.class) {
            final var instance2 = __instance;
            if (instance2 != null)
                return (EmptyConcurrentLinkedQueue<T>) instance2;
            return (EmptyConcurrentLinkedQueue<T>) (__instance = new EmptyConcurrentLinkedQueue<>());
        }
    }
    ...
    @Contract(value = ALWAYS_FAIL_0, pure = true) @Override public T element() { throw new NoSuchElementException("No element could be found in an empty ConcurrentLinkedQueue."); }
    ...
    @Contract(value = ALWAYS_THIS_0, pure = true) @Override public EmptyConcurrentLinkedQueue<T> clone() { return this; }
