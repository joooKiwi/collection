...
/// A bare-bone implementation of a [java NavigableSet][NavigableSet]
...
public class ArrayAsNavigableSet<T extends @Nullable Object>
        extends ArrayAsSortedSet<T>
        implements NavigableSet<T> {

    //#region -------------------- Fields --------------------

    @Serial private static final long serialVersionUID = 6125568236298734687L;

    //#endregion -------------------- Fields --------------------
    //#region -------------------- Constructors --------------------

    /// Create an instance of a [NavigableSet] but allowing `null` in the `reference`.
    /// It also has by default its natural ordering.
    ///
    /// @param reference The array to be the internal structure
    public ArrayAsNavigableSet(final T @NotNull @Unmodifiable [] reference) {
        super(reference);
    }
    ...
    /// Fail to retrieve and remove the first value in the current [ArrayAsNavigableSet]
    ///
    ...
    /// Fail to retrieve and remove the last value in the current [ArrayAsNavigableSet]
    ///
    /// @throws UnsupportedOperationException The method is not supported
    @Contract(ALWAYS_FAIL_0)
