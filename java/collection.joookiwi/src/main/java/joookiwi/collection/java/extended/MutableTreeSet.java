...
public class MutableTreeSet<T>
    ...
    ///
    /// @implNote Use a [Comparable] type on [T] to avoid [ClassCastException]
    public MutableTreeSet(final T @Flow(sourceIsContainer = true, targetIsContainer = true) @NotNull @Unmodifiable [] values) {
        super();
    ...
    ///
    /// @implNote Use a [Comparable] type on [T] to avoid [ClassCastException]
    public MutableTreeSet(final @Flow(sourceIsContainer = true, targetIsContainer = true) @NotNull @Unmodifiable Collection<? extends T> values) {
        super();
        if (values.isEmpty())
            return;
        super.addAll(values);
    ...
    ///
    /// @implNote Use a [Comparable] type on [T] to avoid [ClassCastException]
    public MutableTreeSet(final @Flow(sourceIsContainer = true, targetIsContainer = true) @NotNull @Unmodifiable SortedSet<T> values) {
    ...
    /// Create a mutable instance of [TreeSet]
    /// ordered according to the `comparator` received
    public MutableTreeSet(final @Flow(sourceIsContainer = true, targetIsContainer = true) @NotNull @Unmodifiable SortedSet<? extends T> values,
                          final @Nullable Comparator<? super T> comparator) {
        super(comparator);
        if (values.isEmpty())
            return;
