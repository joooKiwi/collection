...
/// An [ImmutableCollection] with no values (as a [singleton][Singleton] instance)
///
/// @param <T> The type
/// @see joookiwi.collection.java.CollectionConstants#emptyCollection
@Singleton
@NotNullByDefault
public class EmptyCollection<T extends @Nullable Object>
        implements ImmutableCollection<T> {

    //#region -------------------- Singleton usage --------------------

    private static volatile @Nullable EmptyCollection<?> __instance;

    protected EmptyCollection() { super(); }
    ...
    public static <T> EmptyCollection<T> getInstance() {
        ...
        synchronized (EmptyCollection.class) {
            final var instance2 = __instance;
            if (instance2 != null)
                return (EmptyCollection<T>) instance2;
            return (EmptyCollection<T>) (__instance = new EmptyCollection<>());
        }
    }
    ...
    @Contract(value = ALWAYS_1ST_1, pure = true) @Override public <U extends @Nullable Object> U[]      toArray(final U[]                  array) { return array; }
    ...
    @Contract(value = ALWAYS_THIS_0, pure = true) @Override public EmptyCollection<T> clone() { return this; }
