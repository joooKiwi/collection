...
public final class DisableIfExtensionCondition
    ...
    static final String METHOD = "disableIfExtension";
    ...
    @Override public ConditionEvaluationResult evaluateExecutionCondition(final ExtensionContext context) {
        ...
        if (testParentTestValidation(testClass))
            return ConditionEvaluationResult.enabled("The instance where it has the “ExtendWith(DisableIfExtensionCondition.class)” is not disabled since it is about its children.");
        ...
        if (hasNot(extendWithAnnotation.value(), DisableIfExtensionCondition.class))
            return ConditionEvaluationResult.enabled("The child test instance is not desire target because it does not have “@ExtendWith(DisableIfExtensionCondition.class)” present.");
        ...
        if ((boolean) invokeMethod(findMethod(testInstance.getClass(), METHOD).get(), testInstance))
            return ConditionEvaluationResult.disabled("The instance is an extension type.");
        return ConditionEvaluationResult.enabled("The instance is not an extension type.");
    }
