...
/// A Java [TransferQueue] adaptor class from an array
///
/// @param <T> The type
@NotNullByDefault
public final class ArrayAsJavaTransferQueue<T>
        implements TransferQueue<T> {

    public final T[] array;
    public @Nullable TransferQueue<T> reference;

    public ArrayAsJavaTransferQueue(final T[] array) {
        super();
        this.array = array;
    }

    @InitializedOnFirstCall
    public TransferQueue<T> get() {
    ...
    @Contract(ALWAYS_FAIL_0) @Override public ArrayAsJavaTransferQueue<T> clone() { throw new RuntimeException("The method “clone” was not expected to be called in a test."); }
