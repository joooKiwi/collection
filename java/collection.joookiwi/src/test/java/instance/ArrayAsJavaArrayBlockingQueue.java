...
/// A Java [ArrayBlockingQueue] adaptor class from an array
///
/// @param <T> The type
@NotNullByDefault
public final class ArrayAsJavaArrayBlockingQueue<T>
        extends ArrayBlockingQueue<T> {

    @Serial private static final long serialVersionUID = -811863465372607593L;
    public final T[] array;
    public @Nullable ArrayBlockingQueue<T> reference;

    public ArrayAsJavaArrayBlockingQueue(final T[] array) {
        super(1, false);
        this.array = array;
    }
    ...
    public ArrayBlockingQueue<T> get() {
        ...
        if (size == 0)
            return reference = new ArrayBlockingQueue<>(1, false, Arrays.asList(array));
        return reference = new ArrayBlockingQueue<>(size, false, Arrays.asList(array));
    }
    ...
    @Contract(ALWAYS_FAIL_0) @Override public ArrayAsJavaArrayBlockingQueue<T> clone() { throw new RuntimeException("The method “clone” was not expected to be called in a test."); }
