...
/// A Java [CopyOnWriteArraySet] adaptor class from an array
///
/// @param <T> The type
@NotNullByDefault
public final class ArrayAsJavaCopyOnWriteArraySet<T>
        extends CopyOnWriteArraySet<T> {

    @Serial private static final long serialVersionUID = -6152768369093701071L;
    public final T[] array;
    public @Nullable CopyOnWriteArraySet<T> reference;

    public ArrayAsJavaCopyOnWriteArraySet(final T[] array) {
        super();
        this.array = array;
    }

    @InitializedOnFirstCall
    public CopyOnWriteArraySet<T> get() {
        final var value = reference;
        if (value != null)
            return value;
        return reference = new CopyOnWriteArraySet<>(Arrays.asList(array));
    }
    ...
    @Contract(ALWAYS_FAIL_0) @Override public ArrayAsJavaCopyOnWriteArraySet<T> clone() { throw new RuntimeException("The method “clone” was not expected to be called in a test."); }
