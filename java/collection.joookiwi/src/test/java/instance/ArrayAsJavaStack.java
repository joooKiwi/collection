...
/// A Java [Stack] adaptor class from an array
///
/// @param <T> The type
@NotNullByDefault
public final class ArrayAsJavaStack<T extends @Nullable Object>
    ...
    public @Nullable Stack<T> reference;

    public ArrayAsJavaStack(final T[] array) {
        super();
        this.array = array;
    }
    ...
    public Stack<T> get() {
        ...
        final var reference = new Stack<T>();
        final var array = this.array;
        final var size = array.length;
        var index = -1;
        while (++index < size)
            reference.push(array[index]);
    ...
    @Override public T push(final T value) { return get().push(value); }
    ...
    @Contract(ALWAYS_FAIL_0) @Override public ArrayAsJavaStack<T> clone() { throw new RuntimeException("The method “clone” was not expected to be called in a test."); }
