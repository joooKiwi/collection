...
/// A Java [LinkedTransferQueue] adaptor class from an array
///
/// @param <T> The type
@NotNullByDefault
public final class ArrayAsJavaLinkedTransferQueue<T>
        extends LinkedTransferQueue<T> {

    @Serial private static final long serialVersionUID = -8734582104830343493L;
    public final T[] array;
    public @Nullable LinkedTransferQueue<T> reference;

    public ArrayAsJavaLinkedTransferQueue(final T[] array) {
        super();
        this.array = array;
    }

    @InitializedOnFirstCall
    public LinkedTransferQueue<T> get() {
        final var value = reference;
        if (value != null)
            return value;
        return reference = new LinkedTransferQueue<>(Arrays.asList(array));
    }
    ...
    @Override public boolean offer(final T value, final long timeout, final TimeUnit unit) { return get().offer(value, timeout, unit); }

    @Override public void put(final T value) { get().put(value); }
    ...
    @Override public void transfer(final T value) throws InterruptedException { get().transfer(value); }

    @Override public boolean tryTransfer(final T value) { return get().tryTransfer(value); }
    @Override public boolean tryTransfer(final T value, final long timeout, final TimeUnit unit) throws InterruptedException { return get().tryTransfer(value, timeout, unit); }
    ...
    @Contract(ALWAYS_FAIL_0) @Override public ArrayAsJavaLinkedTransferQueue<T> clone() { throw new RuntimeException("The method “clone” was not expected to be called in a test."); }
