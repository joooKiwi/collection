...
/// A Java [ConcurrentLinkedQueue] adaptor class from an array
///
/// @param <T> The type
@NotNullByDefault
public final class ArrayAsJavaConcurrentLinkedQueue<T>
        extends ConcurrentLinkedQueue<T> {

    @Serial private static final long serialVersionUID = 6825978694641404240L;
    public final T[] array;
    public @Nullable ConcurrentLinkedQueue<T> reference;

    public ArrayAsJavaConcurrentLinkedQueue(final T[] array) {
        super();
        this.array = array;
    }

    @InitializedOnFirstCall
    public ConcurrentLinkedQueue<T> get() {
        final var value = reference;
        if (value != null)
            return value;
        return reference = new ConcurrentLinkedQueue<>(Arrays.asList(array));
    }
    ...
    @Contract(ALWAYS_FAIL_0) @Override public ArrayAsJavaConcurrentLinkedQueue<T> clone() { throw new RuntimeException("The method “clone” was not expected to be called in a test."); }
