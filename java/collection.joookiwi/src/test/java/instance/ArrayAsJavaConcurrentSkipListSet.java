...
/// A Java [ConcurrentSkipListSet] adaptor class from an array
///
/// @param <T> The type
@NotNullByDefault
public final class ArrayAsJavaConcurrentSkipListSet<T>
        extends ConcurrentSkipListSet<T> {

    @Serial private static final long serialVersionUID = -7079715203625594485L;
    public final T[] array;
    public @Nullable ConcurrentSkipListSet<T> reference;

    public ArrayAsJavaConcurrentSkipListSet(final T[] array) {
        super();
        this.array = array;
    }

    @InitializedOnFirstCall
    public ConcurrentSkipListSet<T> get() {
        final var value = reference;
        if (value != null)
            return value;
        return reference = new ConcurrentSkipListSet<>(Arrays.asList(array));
    }
    ...
    @Override public NavigableSet<T> headSet(final T to) { return get().headSet(to); }
    @Override public NavigableSet<T> headSet(final T to, final boolean isInclusive) { return get().headSet(to, isInclusive); }
    @Override public NavigableSet<T> tailSet(final T from) { return get().tailSet(from); }
    ...
    @Contract(ALWAYS_FAIL_0) @Override public ArrayAsJavaConcurrentSkipListSet<T> clone() { throw new RuntimeException("The method “clone” was not expected to be called in a test."); }
