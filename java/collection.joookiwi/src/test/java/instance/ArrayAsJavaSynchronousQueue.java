...
/// A Java [SynchronousQueue] adaptor class from an array
///
/// @param <T> The type
@NotNullByDefault
public final class ArrayAsJavaSynchronousQueue<T>
        extends SynchronousQueue<T> {

    @Serial private static final long serialVersionUID = -44635741051067642L;
    public final T[] array;
    public @Nullable SynchronousQueue<T> reference;

    public ArrayAsJavaSynchronousQueue(final T[] array) {
        super();
        this.array = array;
    }

    @InitializedOnFirstCall
    public SynchronousQueue<T> get() {
        final var value = reference;
        if (value != null)
            return value;

        final var reference = new SynchronousQueue<T>();
        final var array = this.array;
        final var size = array.length;
        var index = -1;
        while (++index < size)
            reference.offer(array[index]);
    ...
    @Contract(ALWAYS_FAIL_0) @Override public ArrayAsJavaSynchronousQueue<T> clone() { throw new RuntimeException("The method “clone” was not expected to be called in a test."); }
