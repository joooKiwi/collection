...
/// A Java [LinkedBlockingDeque] adaptor class from an array
///
/// @param <T> The type
@NotNullByDefault
public final class ArrayAsJavaLinkedBlockingDeque<T>
        extends LinkedBlockingDeque<T> {

    @Serial private static final long serialVersionUID = -8841394538422642825L;
    public final T[] array;
    public @Nullable LinkedBlockingDeque<T> reference;

    public ArrayAsJavaLinkedBlockingDeque(final T[] array) {
        super();
        this.array = array;
    }
    ...
    public LinkedBlockingDeque<T> get() {
        ...
        if (value != null)
            return value;
        return reference = new LinkedBlockingDeque<>(Arrays.asList(array));
    }
    ...
    @Override public boolean offerFirst(final T value, final long timeout, final TimeUnit unit) throws InterruptedException { return get().offerFirst(value, timeout, unit); }
    @Override public boolean offerLast(final T value, final long timeout, final TimeUnit unit) throws InterruptedException { return get().offerLast(value, timeout, unit); }

    @Override public void put(final T value) throws InterruptedException { get().put(value); }
    @Override public void putFirst(final T value) throws InterruptedException { get().putFirst(value); }
    @Override public void putLast(final T value) throws InterruptedException { get().putLast(value); }
    ...
    @Contract(ALWAYS_FAIL_0) @Override public ArrayAsJavaLinkedBlockingDeque<T> clone() { throw new RuntimeException("The method “clone” was not expected to be called in a test."); }
