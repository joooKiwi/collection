...
/// A Java [PriorityBlockingQueue] adaptor class from an array
///
/// @param <T> The type
@NotNullByDefault
public final class ArrayAsJavaPriorityBlockingQueue<T>
        extends PriorityBlockingQueue<T> {

    @Serial private static final long serialVersionUID = 6312949259173675520L;
    public final T[] array;
    public @Nullable PriorityBlockingQueue<T> reference;

    public ArrayAsJavaPriorityBlockingQueue(final T[] array) {
        super();
        this.array = array;
    }

    @InitializedOnFirstCall
    public PriorityBlockingQueue<T> get() {
        final var value = reference;
        if (value != null)
            return value;
        return reference = new PriorityBlockingQueue<>(Arrays.asList(array));
    }
    ...
    @Contract(ALWAYS_FAIL_0) @Override public ArrayAsJavaPriorityBlockingQueue<T> clone() { throw new RuntimeException("The method “clone” was not expected to be called in a test."); }
