...
/// A Java [PriorityQueue] adaptor class from an array
///
/// @param <T> The type
@NotNullByDefault
public final class ArrayAsJavaPriorityQueue<T>
        extends PriorityQueue<T> {

    @Serial private static final long serialVersionUID = -3903257747818780370L;
    public final T[] array;
    public @Nullable PriorityQueue<T> reference;

    public ArrayAsJavaPriorityQueue(final T[] array) {
        super();
        this.array = array;
    }

    @InitializedOnFirstCall
    public PriorityQueue<T> get() {
        final var value = reference;
        if (value != null)
            return value;
        return reference = new PriorityQueue<>(Arrays.asList(array));
    }
    ...
    @Contract(ALWAYS_FAIL_0) @Override public ArrayAsJavaPriorityQueue<T> clone() { throw new RuntimeException("The method “clone” was not expected to be called in a test."); }
