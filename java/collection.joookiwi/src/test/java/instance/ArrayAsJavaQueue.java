...
/// A Java [Queue] adaptor class from an array
///
/// @param <T> The type
@NotNullByDefault
public final class ArrayAsJavaQueue<T extends @Nullable Object>
        implements Queue<T> {

    public final T[] array;
    public @Nullable Queue<T> reference;

    public ArrayAsJavaQueue(final T[] array) {
        super();
        this.array = array;
    }
    ...
    @Contract(ALWAYS_FAIL_0) @Override public ArrayAsJavaQueue<T> clone() { throw new RuntimeException("The method “clone” was not expected to be called in a test."); }
