...
/// A Java [SequencedCollection] adaptor class from an array
///
/// @param <T> The type
@NotNullByDefault
public final class ArrayAsJavaSequencedCollection<T extends @Nullable Object>
        implements SequencedCollection<T> {

    public final T[] array;
    public @Nullable SequencedCollection<T> reference;

    public ArrayAsJavaSequencedCollection(final T[] array) {
        super();
        this.array = array;
    }

    @InitializedOnFirstCall
    public SequencedCollection<T> get() {
    ...
    @Override public SequencedCollection<T> reversed() { return get().reversed(); }
    ...
    @Contract(ALWAYS_FAIL_0) @Override public ArrayAsJavaSequencedCollection<T> clone() { throw new RuntimeException("The method “clone” was not expected to be called in a test."); }
