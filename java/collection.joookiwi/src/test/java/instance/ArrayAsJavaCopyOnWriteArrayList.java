...
/// A Java [CopyOnWriteArrayList] adaptor class from an array
///
/// @param <T> The type
@NotNullByDefault
public final class ArrayAsJavaCopyOnWriteArrayList<T extends @Nullable Object>
        extends CopyOnWriteArrayList<T> {

    @Serial private static final long serialVersionUID = -772419687315048002L;
    public final T[] array;
    public @Nullable CopyOnWriteArrayList<T> reference;

    public ArrayAsJavaCopyOnWriteArrayList(final T[] array) {
        super();
        this.array = array;
    }

    @InitializedOnFirstCall
    public CopyOnWriteArrayList<T> get() {
        final var value = reference;
        if (value != null)
            return value;
        return reference = new CopyOnWriteArrayList<>(array);
    }
    ...
    @Contract(ALWAYS_FAIL_0) @Override public ArrayAsJavaCopyOnWriteArrayList<T> clone() { throw new RuntimeException("The method “clone” was not expected to be called in a test."); }
