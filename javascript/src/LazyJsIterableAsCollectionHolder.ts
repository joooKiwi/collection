...
/**
 * A {@link CollectionHolder} having a {@link Iterable Javascript Iterable} value associated to it, but lazily retrieved.
 ...
 */
export class LazyJsIterableAsCollectionHolder<const T = unknown,
    ...
    #lateIterable: () => REFERENCE
    #innerCollection?: JsIterableAsCollectionHolder<T, REFERENCE>

    //#endregion -------------------- Fields --------------------
    //#region -------------------- Constructor --------------------

    public constructor(lateIterable: () => & IterableWithPossibleSize<T> & REFERENCE,)
    public constructor(lateIterable: () => REFERENCE,) {
        super()
        this.#lateIterable = lateIterable
    }
     ...
     */
    protected get _innerCollection(): JsIterableAsCollectionHolder<T, REFERENCE> {
        const value = this.#innerCollection
        if (value != null)
            return value

        const lateIterable = this.#lateIterable()
        this.#lateIterable = FAIL_CALLBACK // We do not need the callback anymore once the value has been retrieved
        return this.#innerCollection = new JsIterableAsCollectionHolder(lateIterable,)
    }
