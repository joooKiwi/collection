 ...
 * @extensionFunction
 */
export function indexOfFirstIndexedOrNull<const T, >(collection: Nullable<| MinimalistCollectionHolder<T> | CollectionHolder<T> | readonly T[]>, predicate: ReverseBooleanCallback<T>, from: NullableNumber = null, to: NullableNumber = null,): NullOrNumber {
    if (collection == null)
        return null
    if (to == null)
        if (from == null)
