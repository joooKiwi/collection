...
export function mapNotNull<const T, const U extends NonNullable<unknown>, >(collection: Nullable<MinimalistCollectionHolder<T>>, transform: ValueIndexWithReturnCallback<T, Nullable<U>>,): CollectionHolder<U> {
    ...
    if (transform.length == 1)
        return new CollectionConstants.LazyGenericCollectionHolder(() => __with1Argument(collection as NonEmptyMinimalistCollectionHolder<T>, transform as (value: T,) => Nullable<U>,),)
...
function __with1Argument<const T, const U extends NonNullable<unknown>, >(collection: NonEmptyMinimalistCollectionHolder<T>, transform: (value: T,) => Nullable<U>,) {
...
function __with2Argument<const T, const U extends NonNullable<unknown>, >(collection: NonEmptyMinimalistCollectionHolder<T>, transform: (value: T, index: number,) => Nullable<U>,) {
    const size = collection.size
    const newArray = [] as U[]
    let index = -1
    while (++index < size) {
