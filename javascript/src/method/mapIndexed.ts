 ...
 * @extensionFunction
 */
export function mapIndexed<const T, const U, >(collection: Nullable<SimplisticCollectionHolder<T>>, transform: IndexValueWithReturnCallback<T, U>,): CollectionHolder<U> {
    if (collection == null)
    ...
    if (transform.length == 1)
        return new CollectionConstants.LazyGenericCollectionHolder(() => __with1Argument(collection as NonEmptySimplisticCollectionHolder<T>, transform as (index: number,) => U,),)
...
function __with1Argument<const T, const U, >(collection: NonEmptySimplisticCollectionHolder<T>, transform: (index: number,) => U,) {
    const size = collection.size
    const newArray = new Array<U>(size,)
    let index = -1
    while (++index < size)
...
function __with2Argument<const T, const U, >(collection: NonEmptySimplisticCollectionHolder<T>, transform: (index: number, value: T,) => U,) {
    const size = collection.size
    const newArray = new Array<U>(size,)
    let index = -1
    while (++index < size)
