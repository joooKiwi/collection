...
describe("CollectionHolderTest (all / any / none)", () => {
    ...
    describe("get() being called", () => {
        ...
        describe("empty ~ collection (by minimalist)", () => {
            describe("all", () => {
                test("(T, number) → true",  () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(EMPTY,).execute(it => it.all(callbackAsTrue2,),) .amountOfCall,).toBe(0,),)
                test("(T) → true",          () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(EMPTY,).execute(it => it.all(callbackAsTrue1,),) .amountOfCall,).toBe(0,),)
                test("() → true",           () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(EMPTY,).execute(it => it.all(callbackAsTrue0,),) .amountOfCall,).toBe(0,),)
                test("(T, number) → false", () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(EMPTY,).execute(it => it.all(callbackAsFalse2,),).amountOfCall,).toBe(0,),)
                test("(T) → false",         () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(EMPTY,).execute(it => it.all(callbackAsFalse1,),).amountOfCall,).toBe(0,),)
                test("() → false",          () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(EMPTY,).execute(it => it.all(callbackAsFalse0,),).amountOfCall,).toBe(0,),)
            },)
            describe("any", () => {
                test('∅',                   () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(EMPTY,).execute(it => it.any(),)                 .amountOfCall,).toBe(0,),)
                test("(T, number) → true",  () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(EMPTY,).execute(it => it.any(callbackAsTrue2,),) .amountOfCall,).toBe(0,),)
                test("(T) → true",          () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(EMPTY,).execute(it => it.any(callbackAsTrue1,),) .amountOfCall,).toBe(0,),)
                test("() → true",           () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(EMPTY,).execute(it => it.any(callbackAsTrue0,),) .amountOfCall,).toBe(0,),)
                test("(T, number) → false", () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(EMPTY,).execute(it => it.any(callbackAsFalse2,),).amountOfCall,).toBe(0,),)
                test("(T) → false",         () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(EMPTY,).execute(it => it.any(callbackAsFalse1,),).amountOfCall,).toBe(0,),)
                test("() → false",          () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(EMPTY,).execute(it => it.any(callbackAsFalse0,),).amountOfCall,).toBe(0,),)
            },)
            describe("none", () => {
                test('∅',                   () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(EMPTY,).execute(it => it.none(),)                 .amountOfCall,).toBe(0,),)
                test("(T, number) → true",  () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(EMPTY,).execute(it => it.none(callbackAsTrue2,),) .amountOfCall,).toBe(0,),)
                test("(T) → true",          () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(EMPTY,).execute(it => it.none(callbackAsTrue1,),) .amountOfCall,).toBe(0,),)
                test("() → true",           () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(EMPTY,).execute(it => it.none(callbackAsTrue0,),) .amountOfCall,).toBe(0,),)
                test("(T, number) → false", () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(EMPTY,).execute(it => it.none(callbackAsFalse2,),).amountOfCall,).toBe(0,),)
                test("(T) → false",         () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(EMPTY,).execute(it => it.none(callbackAsFalse1,),).amountOfCall,).toBe(0,),)
                test("() → false",          () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(EMPTY,).execute(it => it.none(callbackAsFalse0,),).amountOfCall,).toBe(0,),)
            },)
        },)
        describe("empty ~ collection (by normal)", () => {
            describe("all", () => {
                test("(T, number) → true",  () => expect(new CollectionHolder_GetBeingCalledOnNormal(EMPTY,).execute(it => it.all(callbackAsTrue2,),) .amountOfCall,).toBe(0,),)
                test("(T) → true",          () => expect(new CollectionHolder_GetBeingCalledOnNormal(EMPTY,).execute(it => it.all(callbackAsTrue1,),) .amountOfCall,).toBe(0,),)
                test("() → true",           () => expect(new CollectionHolder_GetBeingCalledOnNormal(EMPTY,).execute(it => it.all(callbackAsTrue0,),) .amountOfCall,).toBe(0,),)
                test("(T, number) → false", () => expect(new CollectionHolder_GetBeingCalledOnNormal(EMPTY,).execute(it => it.all(callbackAsFalse2,),).amountOfCall,).toBe(0,),)
                test("(T) → false",         () => expect(new CollectionHolder_GetBeingCalledOnNormal(EMPTY,).execute(it => it.all(callbackAsFalse1,),).amountOfCall,).toBe(0,),)
                test("() → false",          () => expect(new CollectionHolder_GetBeingCalledOnNormal(EMPTY,).execute(it => it.all(callbackAsFalse0,),).amountOfCall,).toBe(0,),)
            },)
            describe("any", () => {
                test('∅',                   () => expect(new CollectionHolder_GetBeingCalledOnNormal(EMPTY,).execute(it => it.any(),)                 .amountOfCall,).toBe(0,),)
                test("(T, number) → true",  () => expect(new CollectionHolder_GetBeingCalledOnNormal(EMPTY,).execute(it => it.any(callbackAsTrue2,),) .amountOfCall,).toBe(0,),)
                test("(T) → true",          () => expect(new CollectionHolder_GetBeingCalledOnNormal(EMPTY,).execute(it => it.any(callbackAsTrue1,),) .amountOfCall,).toBe(0,),)
                test("() → true",           () => expect(new CollectionHolder_GetBeingCalledOnNormal(EMPTY,).execute(it => it.any(callbackAsTrue0,),) .amountOfCall,).toBe(0,),)
                test("(T, number) → false", () => expect(new CollectionHolder_GetBeingCalledOnNormal(EMPTY,).execute(it => it.any(callbackAsFalse2,),).amountOfCall,).toBe(0,),)
                test("(T) → false",         () => expect(new CollectionHolder_GetBeingCalledOnNormal(EMPTY,).execute(it => it.any(callbackAsFalse1,),).amountOfCall,).toBe(0,),)
                test("() → false",          () => expect(new CollectionHolder_GetBeingCalledOnNormal(EMPTY,).execute(it => it.any(callbackAsFalse0,),).amountOfCall,).toBe(0,),)
            },)
            describe("none", () => {
                test('∅',                   () => expect(new CollectionHolder_GetBeingCalledOnNormal(EMPTY,).execute(it => it.none(),)                 .amountOfCall,).toBe(0,),)
                test("(T, number) → true",  () => expect(new CollectionHolder_GetBeingCalledOnNormal(EMPTY,).execute(it => it.none(callbackAsTrue2,),) .amountOfCall,).toBe(0,),)
                test("(T) → true",          () => expect(new CollectionHolder_GetBeingCalledOnNormal(EMPTY,).execute(it => it.none(callbackAsTrue1,),) .amountOfCall,).toBe(0,),)
                test("() → true",           () => expect(new CollectionHolder_GetBeingCalledOnNormal(EMPTY,).execute(it => it.none(callbackAsTrue0,),) .amountOfCall,).toBe(0,),)
                test("(T, number) → false", () => expect(new CollectionHolder_GetBeingCalledOnNormal(EMPTY,).execute(it => it.none(callbackAsFalse2,),).amountOfCall,).toBe(0,),)
                test("(T) → false",         () => expect(new CollectionHolder_GetBeingCalledOnNormal(EMPTY,).execute(it => it.none(callbackAsFalse1,),).amountOfCall,).toBe(0,),)
                test("() → false",          () => expect(new CollectionHolder_GetBeingCalledOnNormal(EMPTY,).execute(it => it.none(callbackAsFalse0,),).amountOfCall,).toBe(0,),)
            },)
        },)

        describe("1 field ~ collection (by minimalist)", () => {
            describe("all", () => {
                test("(T, number) → true",  () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(A,).execute(it => it.all(callbackAsTrue2,),) .amountOfCall,).toBe(1,),)
                test("(T) → true",          () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(A,).execute(it => it.all(callbackAsTrue1,),) .amountOfCall,).toBe(1,),)
                test("() → true",           () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(A,).execute(it => it.all(callbackAsTrue0,),) .amountOfCall,).toBe(0,),)
                test("(T, number) → false", () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(A,).execute(it => it.all(callbackAsFalse2,),).amountOfCall,).toBe(1,),)
                test("(T) → false",         () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(A,).execute(it => it.all(callbackAsFalse1,),).amountOfCall,).toBe(1,),)
                test("() → false",          () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(A,).execute(it => it.all(callbackAsFalse0,),).amountOfCall,).toBe(0,),)
            },)
            describe("any", () => {
                test('∅',                   () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(A,).execute(it => it.any(),)                 .amountOfCall,).toBe(0,),)
                test("(T, number) → true",  () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(A,).execute(it => it.any(callbackAsTrue2,),) .amountOfCall,).toBe(1,),)
                test("(T) → true",          () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(A,).execute(it => it.any(callbackAsTrue1,),) .amountOfCall,).toBe(1,),)
                test("() → true",           () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(A,).execute(it => it.any(callbackAsTrue0,),) .amountOfCall,).toBe(0,),)
                test("(T, number) → false", () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(A,).execute(it => it.any(callbackAsFalse2,),).amountOfCall,).toBe(1,),)
                test("(T) → false",         () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(A,).execute(it => it.any(callbackAsFalse1,),).amountOfCall,).toBe(1,),)
                test("() → false",          () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(A,).execute(it => it.any(callbackAsFalse0,),).amountOfCall,).toBe(0,),)
            },)
            describe("none", () => {
                test('∅',                   () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(A,).execute(it => it.none(),)                 .amountOfCall,).toBe(0,),)
                test("(T, number) → true",  () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(A,).execute(it => it.none(callbackAsTrue2,),) .amountOfCall,).toBe(1,),)
                test("(T) → true",          () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(A,).execute(it => it.none(callbackAsTrue1,),) .amountOfCall,).toBe(1,),)
                test("() → true",           () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(A,).execute(it => it.none(callbackAsTrue0,),) .amountOfCall,).toBe(0,),)
                test("(T, number) → false", () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(A,).execute(it => it.none(callbackAsFalse2,),).amountOfCall,).toBe(1,),)
                test("(T) → false",         () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(A,).execute(it => it.none(callbackAsFalse1,),).amountOfCall,).toBe(1,),)
                test("() → false",          () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(A,).execute(it => it.none(callbackAsFalse0,),).amountOfCall,).toBe(0,),)
            },)
        },)
        describe("1 field ~ collection (by normal)", () => {
            describe("all", () => {
                test("(T, number) → true",  () => expect(new CollectionHolder_GetBeingCalledOnNormal(A,).execute(it => it.all(callbackAsTrue2,),) .amountOfCall,).toBe(1,),)
                test("(T) → true",          () => expect(new CollectionHolder_GetBeingCalledOnNormal(A,).execute(it => it.all(callbackAsTrue1,),) .amountOfCall,).toBe(1,),)
                test("() → true",           () => expect(new CollectionHolder_GetBeingCalledOnNormal(A,).execute(it => it.all(callbackAsTrue0,),) .amountOfCall,).toBe(0,),)
                test("(T, number) → false", () => expect(new CollectionHolder_GetBeingCalledOnNormal(A,).execute(it => it.all(callbackAsFalse2,),).amountOfCall,).toBe(1,),)
                test("(T) → false",         () => expect(new CollectionHolder_GetBeingCalledOnNormal(A,).execute(it => it.all(callbackAsFalse1,),).amountOfCall,).toBe(1,),)
                test("() → false",          () => expect(new CollectionHolder_GetBeingCalledOnNormal(A,).execute(it => it.all(callbackAsFalse0,),).amountOfCall,).toBe(0,),)
            },)
            describe("any", () => {
                test('∅',                   () => expect(new CollectionHolder_GetBeingCalledOnNormal(A,).execute(it => it.any(),)                 .amountOfCall,).toBe(0,),)
                test("(T, number) → true",  () => expect(new CollectionHolder_GetBeingCalledOnNormal(A,).execute(it => it.any(callbackAsTrue2,),) .amountOfCall,).toBe(1,),)
                test("(T) → true",          () => expect(new CollectionHolder_GetBeingCalledOnNormal(A,).execute(it => it.any(callbackAsTrue1,),) .amountOfCall,).toBe(1,),)
                test("() → true",           () => expect(new CollectionHolder_GetBeingCalledOnNormal(A,).execute(it => it.any(callbackAsTrue0,),) .amountOfCall,).toBe(0,),)
                test("(T, number) → false", () => expect(new CollectionHolder_GetBeingCalledOnNormal(A,).execute(it => it.any(callbackAsFalse2,),).amountOfCall,).toBe(1,),)
                test("(T) → false",         () => expect(new CollectionHolder_GetBeingCalledOnNormal(A,).execute(it => it.any(callbackAsFalse1,),).amountOfCall,).toBe(1,),)
                test("() → false",          () => expect(new CollectionHolder_GetBeingCalledOnNormal(A,).execute(it => it.any(callbackAsFalse0,),).amountOfCall,).toBe(0,),)
            },)
            describe("none", () => {
                test('∅',                   () => expect(new CollectionHolder_GetBeingCalledOnNormal(A,).execute(it => it.none(),)                 .amountOfCall,).toBe(0,),)
                test("(T, number) → true",  () => expect(new CollectionHolder_GetBeingCalledOnNormal(A,).execute(it => it.none(callbackAsTrue2,),) .amountOfCall,).toBe(1,),)
                test("(T) → true",          () => expect(new CollectionHolder_GetBeingCalledOnNormal(A,).execute(it => it.none(callbackAsTrue1,),) .amountOfCall,).toBe(1,),)
                test("() → true",           () => expect(new CollectionHolder_GetBeingCalledOnNormal(A,).execute(it => it.none(callbackAsTrue0,),) .amountOfCall,).toBe(0,),)
                test("(T, number) → false", () => expect(new CollectionHolder_GetBeingCalledOnNormal(A,).execute(it => it.none(callbackAsFalse2,),).amountOfCall,).toBe(1,),)
                test("(T) → false",         () => expect(new CollectionHolder_GetBeingCalledOnNormal(A,).execute(it => it.none(callbackAsFalse1,),).amountOfCall,).toBe(1,),)
                test("() → false",          () => expect(new CollectionHolder_GetBeingCalledOnNormal(A,).execute(it => it.none(callbackAsFalse0,),).amountOfCall,).toBe(0,),)
            },)
        },)

        describe("2 fields ~ collection (by minimalist)", () => {
            describe("all", () => {
                test("(T, number) → true",  () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(AB,).execute(it => it.all(callbackAsTrue2,),) .amountOfCall,).toBe(2,),)
                test("(T) → true",          () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(AB,).execute(it => it.all(callbackAsTrue1,),) .amountOfCall,).toBe(2,),)
                test("() → true",           () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(AB,).execute(it => it.all(callbackAsTrue0,),) .amountOfCall,).toBe(0,),)
                test("(T, number) → false", () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(AB,).execute(it => it.all(callbackAsFalse2,),).amountOfCall,).toBe(1,),)
                test("(T) → false",         () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(AB,).execute(it => it.all(callbackAsFalse1,),).amountOfCall,).toBe(1,),)
                test("() → false",          () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(AB,).execute(it => it.all(callbackAsFalse0,),).amountOfCall,).toBe(0,),)
            },)
            describe("any", () => {
                test('∅',                   () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(AB,).execute(it => it.any(),)                 .amountOfCall,).toBe(0,),)
                test("(T, number) → true",  () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(AB,).execute(it => it.any(callbackAsTrue2,),) .amountOfCall,).toBe(1,),)
                test("(T) → true",          () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(AB,).execute(it => it.any(callbackAsTrue1,),) .amountOfCall,).toBe(1,),)
                test("() → true",           () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(AB,).execute(it => it.any(callbackAsTrue0,),) .amountOfCall,).toBe(0,),)
                test("(T, number) → false", () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(AB,).execute(it => it.any(callbackAsFalse2,),).amountOfCall,).toBe(2,),)
                test("(T) → false",         () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(AB,).execute(it => it.any(callbackAsFalse1,),).amountOfCall,).toBe(2,),)
                test("() → false",          () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(AB,).execute(it => it.any(callbackAsFalse0,),).amountOfCall,).toBe(0,),)
            },)
            describe("none", () => {
                test('∅',                   () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(AB,).execute(it => it.none(),)                 .amountOfCall,).toBe(0,),)
                test("(T, number) → true",  () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(AB,).execute(it => it.none(callbackAsTrue2,),) .amountOfCall,).toBe(1,),)
                test("(T) → true",          () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(AB,).execute(it => it.none(callbackAsTrue1,),) .amountOfCall,).toBe(1,),)
                test("() → true",           () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(AB,).execute(it => it.none(callbackAsTrue0,),) .amountOfCall,).toBe(0,),)
                test("(T, number) → false", () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(AB,).execute(it => it.none(callbackAsFalse2,),).amountOfCall,).toBe(2,),)
                test("(T) → false",         () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(AB,).execute(it => it.none(callbackAsFalse1,),).amountOfCall,).toBe(2,),)
                test("() → false",          () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(AB,).execute(it => it.none(callbackAsFalse0,),).amountOfCall,).toBe(0,),)
            },)
        },)
        describe("2 fields ~ collection (by normal)", () => {
            describe("all", () => {
                test("(T, number) → true",  () => expect(new CollectionHolder_GetBeingCalledOnNormal(AB,).execute(it => it.all(callbackAsTrue2,),) .amountOfCall,).toBe(2,),)
                test("(T) → true",          () => expect(new CollectionHolder_GetBeingCalledOnNormal(AB,).execute(it => it.all(callbackAsTrue1,),) .amountOfCall,).toBe(2,),)
                test("() → true",           () => expect(new CollectionHolder_GetBeingCalledOnNormal(AB,).execute(it => it.all(callbackAsTrue0,),) .amountOfCall,).toBe(0,),)
                test("(T, number) → false", () => expect(new CollectionHolder_GetBeingCalledOnNormal(AB,).execute(it => it.all(callbackAsFalse2,),).amountOfCall,).toBe(1,),)
                test("(T) → false",         () => expect(new CollectionHolder_GetBeingCalledOnNormal(AB,).execute(it => it.all(callbackAsFalse1,),).amountOfCall,).toBe(1,),)
                test("() → false",          () => expect(new CollectionHolder_GetBeingCalledOnNormal(AB,).execute(it => it.all(callbackAsFalse0,),).amountOfCall,).toBe(0,),)
            },)
            describe("any", () => {
                test('∅',                   () => expect(new CollectionHolder_GetBeingCalledOnNormal(AB,).execute(it => it.any(),)                 .amountOfCall,).toBe(0,),)
                test("(T, number) → true",  () => expect(new CollectionHolder_GetBeingCalledOnNormal(AB,).execute(it => it.any(callbackAsTrue2,),) .amountOfCall,).toBe(1,),)
                test("(T) → true",          () => expect(new CollectionHolder_GetBeingCalledOnNormal(AB,).execute(it => it.any(callbackAsTrue1,),) .amountOfCall,).toBe(1,),)
                test("() → true",           () => expect(new CollectionHolder_GetBeingCalledOnNormal(AB,).execute(it => it.any(callbackAsTrue0,),) .amountOfCall,).toBe(0,),)
                test("(T, number) → false", () => expect(new CollectionHolder_GetBeingCalledOnNormal(AB,).execute(it => it.any(callbackAsFalse2,),).amountOfCall,).toBe(2,),)
                test("(T) → false",         () => expect(new CollectionHolder_GetBeingCalledOnNormal(AB,).execute(it => it.any(callbackAsFalse1,),).amountOfCall,).toBe(2,),)
                test("() → false",          () => expect(new CollectionHolder_GetBeingCalledOnNormal(AB,).execute(it => it.any(callbackAsFalse0,),).amountOfCall,).toBe(0,),)
            },)
            describe("none", () => {
                test('∅',                   () => expect(new CollectionHolder_GetBeingCalledOnNormal(AB,).execute(it => it.none(),)                 .amountOfCall,).toBe(0,),)
                test("(T, number) → true",  () => expect(new CollectionHolder_GetBeingCalledOnNormal(AB,).execute(it => it.none(callbackAsTrue2,),) .amountOfCall,).toBe(1,),)
                test("(T) → true",          () => expect(new CollectionHolder_GetBeingCalledOnNormal(AB,).execute(it => it.none(callbackAsTrue1,),) .amountOfCall,).toBe(1,),)
                test("() → true",           () => expect(new CollectionHolder_GetBeingCalledOnNormal(AB,).execute(it => it.none(callbackAsTrue0,),) .amountOfCall,).toBe(0,),)
                test("(T, number) → false", () => expect(new CollectionHolder_GetBeingCalledOnNormal(AB,).execute(it => it.none(callbackAsFalse2,),).amountOfCall,).toBe(2,),)
                test("(T) → false",         () => expect(new CollectionHolder_GetBeingCalledOnNormal(AB,).execute(it => it.none(callbackAsFalse1,),).amountOfCall,).toBe(2,),)
                test("() → false",          () => expect(new CollectionHolder_GetBeingCalledOnNormal(AB,).execute(it => it.none(callbackAsFalse0,),).amountOfCall,).toBe(0,),)
            },)
        },)

        describe("N fields ~ collection (by minimalist)", () => {
            describe("all", () => {
                test("(T, number) → true",  () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(ABCD,).execute(it => it.all(callbackAsTrue2,),) .amountOfCall,).toBe(4,),)
                test("(T) → true",          () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(ABCD,).execute(it => it.all(callbackAsTrue1,),) .amountOfCall,).toBe(4,),)
                test("() → true",           () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(ABCD,).execute(it => it.all(callbackAsTrue0,),) .amountOfCall,).toBe(0,),)
                test("(T, number) → false", () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(ABCD,).execute(it => it.all(callbackAsFalse2,),).amountOfCall,).toBe(1,),)
                test("(T) → false",         () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(ABCD,).execute(it => it.all(callbackAsFalse1,),).amountOfCall,).toBe(1,),)
                test("() → false",          () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(ABCD,).execute(it => it.all(callbackAsFalse0,),).amountOfCall,).toBe(0,),)
            },)
            describe("any", () => {
                test('∅',                   () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(ABCD,).execute(it => it.any(),)                 .amountOfCall,).toBe(0,),)
                test("(T, number) → true",  () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(ABCD,).execute(it => it.any(callbackAsTrue2,),) .amountOfCall,).toBe(1,),)
                test("(T) → true",          () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(ABCD,).execute(it => it.any(callbackAsTrue1,),) .amountOfCall,).toBe(1,),)
                test("() → true",           () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(ABCD,).execute(it => it.any(callbackAsTrue0,),) .amountOfCall,).toBe(0,),)
                test("(T, number) → false", () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(ABCD,).execute(it => it.any(callbackAsFalse2,),).amountOfCall,).toBe(4,),)
                test("(T) → false",         () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(ABCD,).execute(it => it.any(callbackAsFalse1,),).amountOfCall,).toBe(4,),)
                test("() → false",          () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(ABCD,).execute(it => it.any(callbackAsFalse0,),).amountOfCall,).toBe(0,),)
            },)
            describe("none", () => {
                test('∅',                   () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(ABCD,).execute(it => it.none(),)                 .amountOfCall,).toBe(0,),)
                test("(T, number) → true",  () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(ABCD,).execute(it => it.none(callbackAsTrue2,),) .amountOfCall,).toBe(1,),)
                test("(T) → true",          () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(ABCD,).execute(it => it.none(callbackAsTrue1,),) .amountOfCall,).toBe(1,),)
                test("() → true",           () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(ABCD,).execute(it => it.none(callbackAsTrue0,),) .amountOfCall,).toBe(0,),)
                test("(T, number) → false", () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(ABCD,).execute(it => it.none(callbackAsFalse2,),).amountOfCall,).toBe(4,),)
                test("(T) → false",         () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(ABCD,).execute(it => it.none(callbackAsFalse1,),).amountOfCall,).toBe(4,),)
                test("() → false",          () => expect(new CollectionHolder_GetBeingCalledOnMinimalist(ABCD,).execute(it => it.none(callbackAsFalse0,),).amountOfCall,).toBe(0,),)
            },)
        },)
        describe("N fields ~ collection (by normal)", () => {
            describe("all", () => {
                test("(T, number) → true",  () => expect(new CollectionHolder_GetBeingCalledOnNormal(ABCD,).execute(it => it.all(callbackAsTrue2,),) .amountOfCall,).toBe(4,),)
                test("(T) → true",          () => expect(new CollectionHolder_GetBeingCalledOnNormal(ABCD,).execute(it => it.all(callbackAsTrue1,),) .amountOfCall,).toBe(4,),)
                test("() → true",           () => expect(new CollectionHolder_GetBeingCalledOnNormal(ABCD,).execute(it => it.all(callbackAsTrue0,),) .amountOfCall,).toBe(0,),)
                test("(T, number) → false", () => expect(new CollectionHolder_GetBeingCalledOnNormal(ABCD,).execute(it => it.all(callbackAsFalse2,),).amountOfCall,).toBe(1,),)
                test("(T) → false",         () => expect(new CollectionHolder_GetBeingCalledOnNormal(ABCD,).execute(it => it.all(callbackAsFalse1,),).amountOfCall,).toBe(1,),)
                test("() → false",          () => expect(new CollectionHolder_GetBeingCalledOnNormal(ABCD,).execute(it => it.all(callbackAsFalse0,),).amountOfCall,).toBe(0,),)
            },)
            describe("any", () => {
                test('∅',                   () => expect(new CollectionHolder_GetBeingCalledOnNormal(ABCD,).execute(it => it.any(),)                 .amountOfCall,).toBe(0,),)
                test("(T, number) → true",  () => expect(new CollectionHolder_GetBeingCalledOnNormal(ABCD,).execute(it => it.any(callbackAsTrue2,),) .amountOfCall,).toBe(1,),)
                test("(T) → true",          () => expect(new CollectionHolder_GetBeingCalledOnNormal(ABCD,).execute(it => it.any(callbackAsTrue1,),) .amountOfCall,).toBe(1,),)
                test("() → true",           () => expect(new CollectionHolder_GetBeingCalledOnNormal(ABCD,).execute(it => it.any(callbackAsTrue0,),) .amountOfCall,).toBe(0,),)
                test("(T, number) → false", () => expect(new CollectionHolder_GetBeingCalledOnNormal(ABCD,).execute(it => it.any(callbackAsFalse2,),).amountOfCall,).toBe(4,),)
                test("(T) → false",         () => expect(new CollectionHolder_GetBeingCalledOnNormal(ABCD,).execute(it => it.any(callbackAsFalse1,),).amountOfCall,).toBe(4,),)
                test("() → false",          () => expect(new CollectionHolder_GetBeingCalledOnNormal(ABCD,).execute(it => it.any(callbackAsFalse0,),).amountOfCall,).toBe(0,),)
            },)
            describe("none", () => {
                test('∅',                   () => expect(new CollectionHolder_GetBeingCalledOnNormal(ABCD,).execute(it => it.none(),)                 .amountOfCall,).toBe(0,),)
                test("(T, number) → true",  () => expect(new CollectionHolder_GetBeingCalledOnNormal(ABCD,).execute(it => it.none(callbackAsTrue2,),) .amountOfCall,).toBe(1,),)
                test("(T) → true",          () => expect(new CollectionHolder_GetBeingCalledOnNormal(ABCD,).execute(it => it.none(callbackAsTrue1,),) .amountOfCall,).toBe(1,),)
                test("() → true",           () => expect(new CollectionHolder_GetBeingCalledOnNormal(ABCD,).execute(it => it.none(callbackAsTrue0,),) .amountOfCall,).toBe(0,),)
                test("(T, number) → false", () => expect(new CollectionHolder_GetBeingCalledOnNormal(ABCD,).execute(it => it.none(callbackAsFalse2,),).amountOfCall,).toBe(4,),)
                test("(T) → false",         () => expect(new CollectionHolder_GetBeingCalledOnNormal(ABCD,).execute(it => it.none(callbackAsFalse1,),).amountOfCall,).toBe(4,),)
                test("() → false",          () => expect(new CollectionHolder_GetBeingCalledOnNormal(ABCD,).execute(it => it.none(callbackAsFalse0,),).amountOfCall,).toBe(0,),)
            },)
        },)
