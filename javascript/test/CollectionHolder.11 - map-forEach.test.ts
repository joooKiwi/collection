...
describe.each(everyInstance,)("%s", ({value: {newInstance,},},) => {
    describe("get() being called", () => {
        ...
        describe("mapIndexed", () => {
            test("() => {}",    () => expect(new CollectionHolderThatCountGetBeingCalled(newInstance(AB,),).execute(it => it.mapIndexed(() => {},).toArray(),).amountOfCall,).toBe(0,),)
            test("(1) => {}",   () => expect(new CollectionHolderThatCountGetBeingCalled(newInstance(AB,),).execute(it => it.mapIndexed(_ => {},).toArray(),).amountOfCall,).toBe(0,),)
            test("(1,2) => {}", () => expect(new CollectionHolderThatCountGetBeingCalled(newInstance(AB,),).execute(it => it.mapIndexed((_1,_2,) => {},).toArray(),).amountOfCall,).toBe(2,),)
        },)
        describe("mapNotNull", () => {
            test("() => {}",    () => expect(new CollectionHolderThatCountGetBeingCalled(newInstance(AB,),).execute(it => it.mapNotNull(() => '',).toArray(),).amountOfCall,).toBe(0,),)
            test("(1) => {}",   () => expect(new CollectionHolderThatCountGetBeingCalled(newInstance(AB,),).execute(it => it.mapNotNull(_ => '',).toArray(),).amountOfCall,).toBe(2,),)
            test("(1,2) => {}", () => expect(new CollectionHolderThatCountGetBeingCalled(newInstance(AB,),).execute(it => it.mapNotNull((_1,_2,) => '',).toArray(),).amountOfCall,).toBe(2,),)
        },)
        describe("mapNotNullIndexed", () => {
            test("() => {}",    () => expect(new CollectionHolderThatCountGetBeingCalled(newInstance(AB,),).execute(it => it.mapNotNullIndexed(() => '',).toArray(),).amountOfCall,).toBe(0,),)
            test("(1) => {}",   () => expect(new CollectionHolderThatCountGetBeingCalled(newInstance(AB,),).execute(it => it.mapNotNullIndexed(_ => '',).toArray(),).amountOfCall,).toBe(0,),)
            test("(1,2) => {}", () => expect(new CollectionHolderThatCountGetBeingCalled(newInstance(AB,),).execute(it => it.mapNotNullIndexed((_1,_2,) => '',).toArray(),).amountOfCall,).toBe(2,),)
        },)
        describe("forEach", () => {
            test("() => {}",    () => expect(new CollectionHolderThatCountGetBeingCalled(newInstance(AB,),).execute(it => it.forEach(() => {},),).amountOfCall,).toBe(0,),)
            test("(1) => {}",   () => expect(new CollectionHolderThatCountGetBeingCalled(newInstance(AB,),).execute(it => it.forEach(_ => {},),).amountOfCall,).toBe(2,),)
            test("(1,2) => {}", () => expect(new CollectionHolderThatCountGetBeingCalled(newInstance(AB,),).execute(it => it.forEach((_1,_2,) => {},),).amountOfCall,).toBe(2,),)
        },)
        describe("forEachIndexed", () => {
            test("() => {}",    () => expect(new CollectionHolderThatCountGetBeingCalled(newInstance(AB,),).execute(it => it.forEachIndexed(() => {},),).amountOfCall,).toBe(0,),)
            test("(1) => {}",   () => expect(new CollectionHolderThatCountGetBeingCalled(newInstance(AB,),).execute(it => it.forEachIndexed(_ => {},),).amountOfCall,).toBe(0,),)
            test("(1,2) => {}", () => expect(new CollectionHolderThatCountGetBeingCalled(newInstance(AB,),).execute(it => it.forEachIndexed((_1,_2,) => {},),).amountOfCall,).toBe(2,),)
        },)
    },)
